...
import { GetProduct } from '../../../app/features/product/application/usecase/get_product'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await new GetProduct(repository).execute({ id: Number(getQuery(event).id) })
